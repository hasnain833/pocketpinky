...
export const ModesSection = () => {
    const modes = [
        ...
        { icon: "🍦", title: "Swirling Mode", description: "IR-specific vetting that screens for real interest", tag: "IR Expertise", tagColor: "bg-[hsl(var(--gold-pale))] text-[hsl(var(--gold))]" },
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-[1100px] mx-auto">
                {modes.map((mode, index) => (
                    <button
                        ...
                        className={`bg-white p-8 rounded text-left w-full transition-all duration-300 border border-[hsl(var(--divider))] hover:shadow-[0_20px_40px_rgba(0,0,0,0.08)] hover:-translate-y-1 hover:border-[hsl(var(--pink-soft))]`}
                    >
